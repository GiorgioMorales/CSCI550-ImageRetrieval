...
def readCIFAR(batch_size=128, trainShuffle=True):
    ...
    else:  # Disable random flips when analyzing similarities
        transform_train = transforms.Compose(
            [transforms.Resize(256),
             transforms.RandomCrop(227),
             transforms.ToTensor(),
    ...
    trainloader = torch.utils.data.DataLoader(trainset, batch_size=batch_size,
                                              shuffle=trainShuffle, num_workers=0)
...
def readMNIST(batch_size=128, trainShuffle=True):
    # Re-set seeds so that the test set always appears in the same order
    torch.manual_seed(7)
    torch.cuda.manual_seed(7)
    torch.backends.cudnn.deterministic = True
